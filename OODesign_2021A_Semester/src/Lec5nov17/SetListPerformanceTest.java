...
{ public static void main(String[] args)
  { // Create a hash set, and test its performance
    ...
    System.out.println("Time for hash set is " +
      getTestTime(set1, 100000) + " milliseconds");
    ...
    System.out.println("Time for linked hash set is " +
      getTestTime(set2, 100000) + " milliseconds");
    ...
    System.out.println("Time for linked list is " +
      getTestTime(list2, 100000) + " milliseconds");
  }
  public static long getTestTime(Collection<Integer> c, int size)
  { long startInnerTime = System.currentTimeMillis();
    // Add numbers 0, 1, 2, ..., size - 1 to the array list
    List<Integer> list = new ArrayList<Integer>();
    for (int i = 0; i < size; i++)     list.add(i);
    Collections.shuffle(list); // Shuffle the array list. shuffle
                               // can be implemented only with List.
    long endInnerTime = System.currentTimeMillis();
    System.out.println("Inner Method Time for " + size + " elemnts " 
    		+ (endInnerTime-startInnerTime) + " milliseconds");
