...
// Why 2 * RECORD_SIZE? because of the UNI-CODE.
// Why 2 * 2 * RECORD_SIZE when we seek the previous record? 
// because we have to go backward 2 records: the current one which
// the pointer is at the end of it, and from there to the beginning
// of the record before the current record. 
// Each one we have to multiply by 2, because of the UNI-CODE.
// For the last record, we are at the end of the file, so 2 * RECORD_SIZE
// to go backward 1 record. 